...
// ============================================================
// CZYTANIE — teksty dobierane do słownika KONKRETNEGO ucznia.
// Nie „poziom A2", tylko policzone pokrycie: ile procent słów w tym tekście już znasz.
// Próg 98 % = czytasz płynnie bez słownika; 95–98 % = da się, ale wolniej; poniżej — szkoda czasu.
// ============================================================
async function viewRead2() {
  clearMain();
  const main = document.querySelector("main");
  const d = await API.get("/api/v2/reading");
  main.append(hero("📖", "Czytanie",
    `Teksty dobrane do Twojego słownika · znasz ${d.known_words} słów`, "teal"));

  main.append(el("div", { class: "card" },
    el("p", { class: "muted small" },
      "Aplikacja liczy, ile procent słów w każdym tekście już znasz. Przy 98 % czytasz " +
      "płynnie i bez słownika — i właśnie wtedy czytanie uczy najwięcej. Poniżej 95 % " +
      "tekst męczy zamiast uczyć, dlatego oznaczamy go na czerwono."),
    d.ready
      ? el("div", { class: "rd-ready" }, `✔ ${d.ready} ${d.ready === 1 ? "tekst gotowy" : "teksty gotowe"} do czytania`)
      : el("div", { class: "muted small" }, "Jeszcze żaden tekst nie jest w pełni „na Twoją miarę” — poucz się słówek i wróć.")));

  const box = el("div", { class: "card" });
  d.texts.forEach(t => {
    const pct = Math.round(t.coverage * 100);
    box.append(el("button", { class: "rd-item rd-" + t.state, onclick: () => runRead2(t.id) },
      el("span", { class: "rd-emoji" }, t.emoji),
      el("div", { class: "rd-body" },
        el("b", {}, t.title, t.done ? " ✓" : ""),
        el("div", { class: "muted small" }, `${t.words} słów · ${t.minutes} min · ${t.level}`),
        el("div", { class: "rd-cov" },
          el("div", { class: "rd-bar" }, el("div", { class: "rd-fill", style: `width:${pct}%` })),
          el("span", { class: "rd-pct" }, pct + "%")),
        el("div", { class: "muted small" },
          t.state === "easy" ? "znasz prawie wszystkie słowa — czytaj śmiało"
            : t.state === "ok" ? `${t.unknown} nieznanych słów — da się, ale wolniej`
            : `${t.unknown} nieznanych słów — na razie za trudny`))));
  });
  main.append(box);
  main.append(el("div", { class: "v2-foot" },
    el("button", { class: "btn ghost mini", onclick: viewToday }, "← Ekran główny")));
...
async function runRead2(tid) {
  const t = await API.get("/api/v2/reading/" + tid);
  clearMain();
  const main = document.querySelector("main");
  const box = el("div", { class: "card" });
  main.append(box);
  enterFocus({ title: t.emoji + " " + t.title, subtitle: `${t.words} słów · ${t.minutes} min`,
    theme: "teal", onExit: () => { stopSpeaking(); exitFocus(); viewRead2(); } });
  ...
  function reading() {
    box.innerHTML = "";
    box.append(el("div", { class: "v2-crumb" },
      `znasz ${Math.round(t.coverage * 100)}% słów w tym tekście`));
    const wrap = el("div", { class: "rd-text" });
    t.lines.forEach(line => {
      const p = el("p", { class: "rd-line" });
      // każde zdanie ma lektora, każde nieznane słowo jest klikalne
      p.append(el("button", { class: "mini-tts", onclick: () => speak(line) }, "🔊"), " ");
      line.split(/(\s+)/).forEach(chunk => {
        const clean = chunk.toLowerCase().replace(/[^a-z']/g, "");
        if (clean && unknown.has(clean)) {
          const b = el("button", { class: "rd-unknown", onclick: e => {
            e.stopPropagation();
            showGloss(b, clean, chunk);
          } }, chunk);
          p.append(b);
        } else p.append(chunk);
      });
    ...
    box.append(wrap,
      el("div", { class: "muted small", style: "margin-top:10px" },
        "Podkreślone słowa to te, których jeszcze nie miałeś. Dotknij, żeby zobaczyć znaczenie " +
        "i dodać do utrwalenia. Nie tłumacz całości — czytaj dalej, sens zwykle wychodzi z kontekstu."),
      el("div", { class: "fb-btns", style: "margin-top:14px" },
        el("button", { class: "btn ghost", onclick: () => speak(t.lines.join(" ")) }, "🔊 Przeczytaj całość"),
        el("button", { class: "btn ghost", onclick: stopSpeaking }, "⏹ Stop"),
        el("button", { class: "btn primary", onclick: quiz }, "Rozumiem — pytania →")));
  }

  function showGloss(btn, clean, original) {
    const pl = t.gloss[clean];
    const pop = el("div", { class: "rd-gloss" },
      el("b", {}, original), pl ? " — " + pl : " — (brak w słowniku)", " ",
      el("button", { class: "mini-tts", onclick: () => speak(original) }, "🔊"),
      el("button", { class: "btn ghost mini", onclick: async e => {
        e.stopPropagation();
        try {
          await API.post("/api/hardwords/add", { en: clean, pl: pl || "", ctx_en: "" });
          toast("🔥 Dodano do utrwalenia: " + clean);
          btn.classList.add("rd-added");
        } catch (err) { toast(String(err.message || err), true); }
      } }, "🔥 Do utrwalenia"));
    const old = box.querySelector(".rd-gloss");
    if (old) old.remove();
    btn.after(pop);
  }
  ...
  function quiz() {
    ...
    step();
    function step() {
      if (i >= t.questions.length) return finish();
      const q = t.questions[i];
      box.innerHTML = "";
      focusProgress(i, t.questions.length, "");
      box.append(el("div", { class: "v2-crumb" }, "Pytanie " + (i + 1) + " z " + t.questions.length),
      ...
      q.options.forEach((o, k) => opts.append(el("button", { class: "option", onclick: () => {
        ...
        box.append(el("div", { class: "feedback " + (ok ? "fb-good" : "fb-bad") },
          el("div", { class: "fb-head" }, ok ? "✔ Dobrze" : "✘ Poprawnie: " + q.options[q.answer])),
          el("div", { class: "fb-btns" },
            el("button", { class: "btn primary", onclick: () => { i++; step(); } }, "Dalej →"),
            el("button", { class: "btn ghost", onclick: reading }, "← Wróć do tekstu")));
      } }, o)));
      box.append(opts);
    }
    async function finish() {
      const r = await API.post("/api/v2/reading/" + tid, { correct: good, total: t.questions.length });
      exitFocus();
      clearMain();
      const m2 = document.querySelector("main");
      m2.append(hero("📖", "Tekst przeczytany",
        `Rozumienie: ${good} z ${t.questions.length}`, "teal"));
      m2.append(el("div", { class: "card" },
        el("p", {}, good === t.questions.length
          ? "Pełne rozumienie — to znak, że tekst był w sam raz. Następny może być odrobinę trudniejszy."
          : "Nie wszystko wyszło, ale to nic złego: przy czytaniu ekstensywnym liczy się ilość, nie perfekcja."),
        el("p", { class: "muted small" }, `Przeczytanych tekstów: ${r.texts_done}.`),
        el("div", { class: "fb-btns" },
          el("button", { class: "btn primary", onclick: viewRead2 }, "→ Kolejny tekst"),
          el("button", { class: "btn ghost", onclick: viewToday }, "Ekran główny"))));
    }
  }
}
